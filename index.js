...
app.post("/api/chat", async (req, res) => {
    ...
    if (!message) {
        return res.status(400).json({ error: 'Message is required' }) // send an error response if the message is not provided
    }

    try {
        const result = await model.generateContent(message) // process the message and waiting for Gemini to process it
...
// ----- Serve static files -----
// This should come after API route definitions to ensure API routes are prioritized.
app.use(express.static('public')) // serve static files from the 'public' directory

app.listen(port, () => {    
    console.log(`Server is running on http://localhost:${port}`) // log a message to the console when the server starts
    console.log("Frontend Anda sekarang dapat diakses dari alamat yang sama.");
